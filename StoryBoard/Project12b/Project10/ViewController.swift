...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let data = UserDefaults.standard.object(forKey: "people") as? Data{
            if let decodedPeople = try? JSONDecoder().decode([Person].self, from: data){
                people = decodedPeople
            }
            else{
                print("Decode data to people failed")
            }
        }
        else{
            print("Can't find UserDefault save sata for the key 'people'")
        }
    }
